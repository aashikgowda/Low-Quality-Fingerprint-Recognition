...
function [normim, mask, maskind] = ridgesegment(im, blksze, thresh)
    ...
    im = im2double(im);  % normalise to have zero mean, unit std dev
...
for i=2:1:size(mask,1)-1
   for j=2:1:size(mask,2)-1
       sum = mask(i-1,j-1)+mask(i-1,j)+mask(i-1,j+1)+mask(i,j-1) ...
...
mask = mask(2:1:size(mask,1)-1,2:1:size(mask,2)-1);
maskind = find(mask);
